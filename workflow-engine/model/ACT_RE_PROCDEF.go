...
func (t *Procdef) TableName() string {
	return "procdef"
}
...
func GetProcdefLatestByNameAndCompany(name, company string) (*Procdef, error) {
	var p []*Procdef
	err := GetDB().Where("name=? and company=?", name, company).Order("version desc").Find(&p).Error
	if err != nil || len(p) == 0 {
		return nil, err
	}
	return p[0], err
...
func GetProcdefByID(id int) (*Procdef, error) {
	var p = &Procdef{}
	err := GetDB().Where("id=?", id).Find(p).Error
	return p, err
}
...
func DelProcdefByID(id int) error {
	err := GetDB().Where("id = ?", id).Delete(&Procdef{}).Error
	return err
}
...
func FindProcdefsWithCountAndPaged(pageIndex, pageSize int, maps map[string]interface{}) (datas []*Procdef, count int64, err error) {
	err = GetDB().Select("id,name,version,userid,deploy_time").Where(maps).Offset((pageIndex - 1) * pageSize).Limit(pageSize).Find(&datas).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		return nil, 0, err
	}
	err = GetDB().Model(&Procdef{}).Where(maps).Count(&count).Error
	if err != nil {
		return nil, 0, err
	}
